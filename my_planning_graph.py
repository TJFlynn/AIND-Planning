...
class PlanningGraph():
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a_1_action in node_a1.action.effect_add:
            for a_2_action in node_a2.action.effect_rem:
                if a_1_action == a_2_action:
                    return True
                
        for a_1_action in node_a1.action.effect_rem:
            for a_2_action in node_a2.action.effect_add:
                if a_1_action == a_2_action:
                    return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a_1_action in node_a1.action.effect_rem:
            for a_2_condition in node_a2.action.precond_pos:
                if a_1_action == a_2_condition:
                    return True
                
        for a_2_action in node_a2.action.effect_rem:
            for a_1_condition in node_a1.action.precond_pos:
                if a_2_action == a_1_condition:
                    return True
        return False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        number_of_mutex = 0
        length1 = len(node_s1.parents)
        length2 = len(node_s2.parents)
        pair_length = min(length1, length2) #find smallest value for pairs, since this is pairwise exclusion
        
        for parent_1 in node_s1.parents:
            for parent_2 in node_s2.parents:
                if parent_1.is_mutex(parent_2):
                    number_of_mutex += 1
        if number_of_mutex == pair_length:
            return True
        return False
    ...
    def h_levelsum(self) -> int:
        ...
        #create a cache to avoid even deeper levels of if and for nesting
        cache = {}
        for level_num, level in enumerate(self.s_levels):
            cache[level_num] = []
            for node in level:
                if node.is_pos:
                    cache[level_num].append(node.symbol)
               
        level_sum = 0
        goals = list(self.problem.goal)
        
        for num, symb_list in cache.items():
